...
blocked_users = set()
qr_mode       = False
pending_qr    = None  # file_id

# ── состояния ──────────────────────────────────────────────────────────────
(
    DEP_CASINO, DEP_ID, DEP_BANK, DEP_AMOUNT,
...
def cancel_kb():
    return ReplyKeyboardMarkup(
        [[KeyboardButton("❌ Операция отменена")]],
        resize_keyboard=True
    )
...
def dep_bank_ikb():
    ...
    rows = [[InlineKeyboardButton(f"🏦 {b}  🚫 Не доступно", callback_data=f"depbank_{b}")] for b in banks]
...
def wd_bank_ikb():
    ...
    for i in range(0, len(banks), 2):
        row = [InlineKeyboardButton(f"🏦 {n}", callback_data=f"wdbank_{c}") for n,c in banks[i:i+2]]
        rows.append(row)
    return InlineKeyboardMarkup(rows)
...
async def dep_id(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🏦 Выберите банк для оплаты:", reply_markup=dep_bank_ikb())
    return DEP_BANK

async def dep_bank_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
...
async def dep_bank_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # если пользователь написал вместо кнопки — просим ввести сумму
    await update.message.reply_text(
...
async def dep_amount_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not update.message.text.isdigit():
        ...
        return DEP_AMOUNT
    await _process_deposit(update, ctx, update.message.text, update.message)
    return ConversationHandler.END
...
async def _process_deposit(update, ctx, amount, msg_obj):
    ...
    if qr_mode and pending_qr:
        await msg_obj.reply_photo(photo=pending_qr, caption=caption,
                                  parse_mode="HTML", reply_markup=main_kb())
    else:
    ...
    notif = (
        ...
        f"🎰 Казино: {casino}\n🎫 ID счёта: <code>{casino_id}</code>\n💰 Сумма: {amount} сом"
    )
    await notify_admin(app, notif, uid)
...
async def wd_bank(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    bank_map = {"kompanyon":"Компаньон","obank":"O банк","bakai":"Bakai",
                "balance":"Balance.kg","megapay":"MegaPay","mbank":"MBank"}
    bank = bank_map.get(q.data.split("wdbank_")[1], q.data)
...
async def wd_qr(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"6. Получить Код!\n7. Отправить его нам\n\n🆔 Введите ID вашего счёта {casino}:"
    )
    return WD_CID
...
async def cmd_qr(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    qr_mode = True
    await update.message.reply_text(
        "📲 Режим QR включён.\nОтправьте фото QR-кода для пользователей:"
    )
    return ADM_QR
...
# ════════════════════════════════════════════════════════════════════════════
#  ADMIN кнопки блок/разблок/написать
# ════════════════════════════════════════════════════════════════════════════
async def cb_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ablock_"):
        ...
        ctx.user_data["cob_target"] = uid
        await q.message.reply_text(
            f"✍️ Введите сообщение для пользователя {uid}\n"
            f"Или используйте: /cob {uid} текст"
        )
        await q.answer()
...
def main():
    ...
    dep_conv = ConversationHandler(
        ...
        states={
            ...
            DEP_BANK:   [
                CallbackQueryHandler(dep_bank_cb, pattern="^depbank_"),
    ...
    app.add_handler(CallbackQueryHandler(cb_admin, pattern="^a(block|unblock|write)_"))
